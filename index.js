...
function handler(event){
    ...
    let possition = objData.y - window.screen.height/2

    obj.style.transform = `translatex(${-possition}px)`
}

window.addEventListener("scroll",handler)

const obj2 = document.querySelector(".cub")
function handler2(event){
    const obj2Data = obj.getBoundingClientRect()

    let possition2 = obj2Data.y - window.screen.height/2

    obj2.style.transform = `translatex(${possition2}px)`
}
